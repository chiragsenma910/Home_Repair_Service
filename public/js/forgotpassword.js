document.addEventListener("DOMContentLoaded", function() {
    ...
    const professionDropdown = document.getElementById('yourprofession');
    ...
    const roleDropdown = document.querySelector('select[name="yourRole"]');
    if (roleDropdown) {
        roleDropdown.addEventListener('change', toggleProfessionDropdown);
    }
    ...
    forgotOtpForm.addEventListener("submit", function(event) {
        ...
        // Extract email and role from the form
        const emailInput = document.querySelector('#forgot-otp-form input[name="email"]');
        const email = emailInput.value.trim();
        const roleSelect = document.querySelector('#forgot-otp-form select[name="yourRole"]');
        const role = roleSelect.value;

        // Extract selected profession if role is "Worker"
        let profession = null;
        if (role === 'worker') {
            profession = professionDropdown.value;
        }
        ...
        fetch('/forgot-password/send-otp', {
            ...
            body: JSON.stringify({ role, email, profession })
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                // Start the countdown timer for showing the resend button after 10 seconds
    ...
    verifyOtpForm.addEventListener("submit", function(event) {
        ...
        const otpInput = document.querySelector('#verify-otp-form input[name="otp"]');
        ...
        .then(data => {
            if (data.success) {
                ...
                const verifyOtpCard = document.getElementById("verify-otp");
                const setNewPasswordCard = document.getElementById("set-new-password");
    ...
    resendButton.addEventListener("click", function(event) {
        ...
        fetch('/forgot-password/send-otp', {
            ...
            body: JSON.stringify({ yourRole: 'user', email: document.querySelector('#forgot-otp-form input[name="email"]').value.trim() })
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
    ...
    setPasswordForm.addEventListener("submit", function(event) {
        ...
        const passwordInput = document.querySelector('#set-password-form input[name="password"]');
        const confirmPasswordInput = document.querySelector('#set-password-form input[name="confirm_password"]');
        const password = passwordInput.value.trim();
        const confirmPassword = confirmPasswordInput.value.trim();
        const email = document.querySelector('#forgot-otp-form input[name="email"]').value.trim();
        const yourRole = document.querySelector('#forgot-otp-form select[name="yourRole"]');
...
function toggleDropdown() {
    var roleDropdown2 = document.getElementById("yourRole");
    var professionGroup2 = document.getElementById('your-profession');
    if (roleDropdown2.value === "worker") {
        professionGroup2.style.display = "block";
      } 
      else {
        professionGroup2.style.display = "none";
      }
 }


 document.addEventListener("DOMContentLoaded", function() {
    const eyeIcon1 = document.getElementById("eye-icon7");
    const eyeIcon2 = document.getElementById("eye-icon8");
    const passwordInput = document.getElementById("password7");
    const confirmPassword = document.getElementById("password8");
